...
class AggregateMemorySavingTask extends AbstractDao {

    public function insertData($dbh, $fromYyyymm, $toYyyymm) {
		...
		$maxCount = 0;
		$minCount = 99999999;
		echo $fromYyyymm."-".$toYyyymm."\n";
		//for($i = 0; $i < count($tCountList); $i++){
		$order = 'ORDER BY WORD_ID,SITE_ID,DATE';
		$stmt = $dbh->prepare("SELECT * FROM T_COUNT WHERE DATE_FORMAT(DATE, '%Y%m') BETWEEN '" . $fromYyyymm . "' AND '" . $toYyyymm . "' " . $order . ';');
		...
		while ($row = $stmt->fetch()) {
		    $countData = self::getRowData($row);
//echo var_dump($countData);
